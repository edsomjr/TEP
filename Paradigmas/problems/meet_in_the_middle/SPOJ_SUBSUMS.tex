...
\begin{frame}[fragile]{Solução $O(2^{N/2}\log N)$}
    ...
    \begin{itemize}
        ...
        \item Se $S_2$ estiver ordenado, este intervalo de valores pode ser computado por meio
            de duas buscas binários, ou através das funções \code{cpp}{lower_bound} e
...
\begin{frame}[fragile]{Solução $O(2^{N/2}\log N)$}
    \inputsnippet{cpp}{1}{21}{codes/SPOJ_SUBSUMS.cpp}
\end{frame}

\begin{frame}[fragile]{Solução $O(2^{N/2}\log N)$}
    \inputsnippet{cpp}{22}{42}{codes/SPOJ_SUBSUMS.cpp}
\end{frame}

\begin{frame}[fragile]{Solução $O(2^{N/2}\log N)$}
    \inputsnippet{cpp}{43}{63}{codes/SPOJ_SUBSUMS.cpp}
\end{frame}
