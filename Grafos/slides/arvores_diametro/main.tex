...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[] (info) at (7.0, 6.0) { \bbtext{Sendo $T$ conectada, existe ao menos um caminho de $r$ no caminho de $u$ a $v$} };
	...
	\node[] (info2) at (7.0, 5.5) { \bbtext{para um $s$ no caminho de $p$ a $q$} };
...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[] (info2) at (7.0, 5.25) { $\displaystyle \dist(v, s) + \dist(s, q) = \dist(v, q) < \dist(p, q) = \dist(p, s) + \dist(s, v)$ };
...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[anchor=west] (output) at (0.5, 5.5) { \bbemph{Saída:} \bbtext{o diâmetro $D$ do grafo} };
